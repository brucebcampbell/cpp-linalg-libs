     ...
     int& ralen)
{
    ...
    ra(2) = c.n;
    ra(3) = c.k;
    ap::vmove(&ra(4), 1, &c.x(0), 1, ap::vlen(4,4+c.n-1));
    ap::vmove(&ra(4+c.n), 1, &c.c(0), 1, ap::vlen(4+c.n,4+c.n+(c.k+1)*(c.n-1)-1));
}
...
OUTPUT PARAMETERS:
  ...
  -- ALGLIB --
     Copyright 17.08.2009 by Bochkanov Sergey
*************************************************************************/
void spline1dunserialize(const ap::real_1d_array& ra, spline1dinterpolant& c)
{

    ap::ap_error::make_assertion(ap::round(ra(1))==spline1dvnum, "Spline1DUnserialize: corrupted array!");
    c.n = ap::round(ra(2));
    c.k = ap::round(ra(3));
    c.x.setlength(c.n);
    c.c.setlength((c.k+1)*(c.n-1));
    ap::vmove(&c.x(0), 1, &ra(4), 1, ap::vlen(0,c.n-1));
    ap::vmove(&c.c(0), 1, &ra(4+c.n), 1, ap::vlen(0,(c.k+1)*(c.n-1)-1));
}
