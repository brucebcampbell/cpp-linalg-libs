        ...
        for(n = 1; n <= 10; n++)
        {
            for(m = 1; m <= 10; m++)
            {
                ...
                for(tasktype = 0; tasktype <= 4; tasktype++)
                {
                    ...
                    for(i = 1; i <= n; i++)
                    {
                        tmp = ap::vdotproduct(&h(i, 1), 1, &x(1), 1, ap::vlen(1,n));
                        if( i==1 )
                        {
                ...
                for(i = 1; i <= m; i++)
                {
                    for(j = 1; j <= n; j++)
                    {
                        tmp = ap::vdotproduct(&h(i, 1), 1, &a(1, j), a.getstride(), ap::vlen(1,m));
                        c(i,j) = tmp;
                    }
                }
                err = 0;
                ...
                for(i = 1; i <= m; i++)
                {
                    for(j = 1; j <= n; j++)
                    {
                        tmp = ap::vdotproduct(&a(i, 1), 1, &h(1, j), h.getstride(), ap::vlen(1,n));
                        c(i,j) = tmp;
                    }
                }
                err = 0;
